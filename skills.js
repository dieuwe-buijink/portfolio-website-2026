...
function openModal(skill) {
    ...
    function renderMedia(container, src, altText, opts = {}) {
        ...
        if(ext === 'mp4') {
            ...
            video.setAttribute('controls', ''); video.setAttribute('playsinline', '');
            video.style.width = '100%'; video.style.height = '100%'; video.style.objectFit = opts.fit || 'cover';
        ...
        img.style.borderRadius = '12px'; img.style.display = 'block'; img.style.background = '#000';
        img.onload = () => {
            ...
            if (opts.fit === 'contain') {
                if (desiredHeight <= maxHeight) {
                    ...
                    container.style.height = `${Math.round(maxHeight)}px`; container.style.overflow = 'auto';
                    img.style.width = 'auto'; img.style.maxHeight = '100%'; img.style.objectFit = 'contain'; img.style.margin = '0 auto';
                }
            } else {
